...
async def get_result(idea, paperIds, use_retrieval, ablation, save_path):
        ...
        for i in list(iteration_metadata.keys())[::-1]:
            ...
            if papers is not None and papers.get('data', None) is not None:
                for paper in papers['data']:
                    temp = {k: paper.get(k) for k in ['paperId', 'corpusId', 'url', 'title', 'abstract',
                                                        'venue', 'year', 'citationCount', 'fieldsOfStudy', 'authors']}
                    aiscientist_papers.append(temp)
                break

        os.makedirs(f"{base_path}/aiscientist", exist_ok=True)
        with open(aisci_path, "w") as f:
            json.dump({"chat_history": chat_history, "iteration_metadata": iteration_metadata}, f)
    ...
    if not os.path.exists(ideanoveltychecker_path):
        ...
        for file in ["input_most_relevant_papers", "most_relevant_papers", "embedding_ranked", "snippet_papers", "keyword_papers"]:
        ...
        metadata = {
            ...
            "output": {file: ideanc_output["trace"][file] for file in ["idea_keywords", "title_keywords", "idea_priority_facets"]}
        }
...
def main():
    ...
    results = asyncio.run(get_result(args.idea, args.papers, args.use_retrieval, args.ablation, args.save_path))
