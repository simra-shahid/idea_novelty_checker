...
async def embeddingFiltering(idea, paper_embeddings_metadata, max_papers_specter=100):
    ...
    idea_embedding_metadata = await get_embeddings_ideapapers(
        [{"corpusId": "-1", "title": "", "abstract": idea}]
    )  # shape (1, D)
    ...
    for corpus_id, paper_data in tqdm(
        paper_embeddings_metadata.items(),
        total=len(paper_embeddings_metadata),
        desc="Checking if all papers contain embeddings...",
        disable=True,
    ):
        try:
            if "embedding" in paper_data:
                if isinstance(paper_data["embedding"], dict):
                    paper_embeddings.append(
                        np.array(paper_data["embedding"]["vector"]).reshape(1, -1)
                    )
                else:
                    paper_embeddings.append(
                        np.array(paper_data["embedding"]).reshape(1, -1)
                    )
    ...
    paper_similarities = compute_similarities_in_batches(
        paper_embeddings, idea_embedding
    )
    ...
    paper_similarities_df = paper_similarities_df.sort_values(
