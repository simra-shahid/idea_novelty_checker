...
async def get_most_relevant_papers(
    ...
    await rank_gpt_filter(item, similar_documents, os.getenv('RANKGPT_MODEL'),  os.getenv('RANKGPT_VARIANT'), idea_priority_facets)

    similar_documents.sort_values(f'rankGPT_{os.getenv("RANKGPT_VARIANT")}', inplace=True) 
