"""
Implements an async pipeline to fetch papers, filter them with embedding-based ranking,
then optionally rerank them with LLM.
"""
import asyncio
import pandas as pd
import numpy as np 
...
async def get_priority_facets(idea): 
    ...
    output, _ = client.chat(
        ...
        messages=[{"role": "system", "content": priority_prompt}] if isinstance(priority_prompt, str) else priority_prompt, 
...
async def get_query_based_papers(idea, seed_papers, max_papers_specter=100):
    ...
    most_similar_documents = await embeddingFiltering(idea, input_papers, max_papers_specter)

    query_retrieval_trace.update({
        "embedding_ranked": most_similar_documents
    })

    return query_retrieval_trace


async def get_most_relevant_papers(
    idea, seed_papers, max_papers_specter=100
):
    ...
    if len(most_similar_documents_filtered) != len(query_retrieval_trace["embedding_ranked"]):
        removed = len(query_retrieval_trace["embedding_ranked"]) - len(most_similar_documents_filtered)
    ...
    await rank_gpt_filter(item, similar_documents, os.getenv('RANKGPT_MODEL'), "priority", idea_priority_facets)

    similar_documents.sort_values('rankGPT_priority', inplace=True) 
